...
Updater& Updater::onProgress(THandlerFunction_Progress fn) {
...
bool Updater::begin(size_t size, int command, int ledPin, uint8_t ledOn) {
    if(_size > 0){
...
/*        _partition = esp_ota_get_next_update_partition(NULL);
        ...
        esp_partition_iterator_t ota_partition;

        if(String(running->label)=="app0"){             // check what parttion is running
            ota_partition = esp_partition_find(ESP_PARTITION_TYPE_APP, ESP_PARTITION_SUBTYPE_APP_OTA_1 , NULL);
        }else{
            ota_partition = esp_partition_find(ESP_PARTITION_TYPE_APP, ESP_PARTITION_SUBTYPE_APP_OTA_0 , NULL);
        }
        
        _partition = esp_partition_get(ota_partition);
        esp_partition_iterator_release(ota_partition);  // release partition intertor
...
void Updater::_abort(uint8_t err){
    _reset();
    _error = err;
}
...
bool Updater::_verifyHeader(uint8_t data) {
    if(_command == U_FLASH) {
...
bool Updater::end(bool evenIfRemaining){
    if(hasError() || _size == 0){
        return false;
    }
...
size_t Updater::write(uint8_t *data, size_t len) {
...
size_t Updater::writeStream(Stream &data) {
    size_t written = 0;
    size_t toRead = 0;
    if(hasError() || !isRunning())
        return 0;
...
void Updater::printError(Stream &out){
